...
function plugin(options){
      ...
      return function(data){
        each(data,
          entryProcessor({
            ...
            filenameDate         : options.entry_filename_date_format,
    ...
    function entryProcessor(options) {
      return function (entry, entryProcessedCallback){
          ...
          if (options.filenameField) {
            ...
            params.forEach(function(element, index){
              ...
              if (entry[prefix] && void 0 !== entry[prefix][item]) {
                if (item === "pubDate") {
                  var date = Date.parse(entry[prefix][item]);
                  var datef = options.filenameDate ? options.filenameDate : 'YYYY/MM/DD';
                  var d = moment(date).format(datef)
                  pattern = pattern.replace(":" +element, d);
                } else {
        ...
        file = {
          ...
          data        : entry,
          date        : dateStandardizer(entry),
          id          : entry.sys.id,
          contentType : contentType,
          template    : options.template,
          parent      : options.parent,
          slug        : entry.fields.slug ? entry.fields.slug : entry.sys.id
        };
  ...
  //
  // dateStandardizer: takes a Contentful entry (as returned by the API)
  //    and determines which date to use as THE date in Metalsmith.
  //  returns a JS Date object.

  function dateStandardizer(entry){
    var e = entry;
    debug('Running dateStandardizer on entry: %s',e.sys.id);
    var created = moment(e.sys.createdAt);
    debug('dateStandardizer [%s] > created: %s',e.sys.id,created);
    var updated = moment(e.sys.updatedAt);
    debug('dateStandardizer [%s] > updated: %s',e.sys.id,updated);
    var published = e.fields.pubDate?moment(e.fields.pubDate):'pubDate is not set';
    debug('dateStandardizer [%s] > published: %s',e.sys.id,published);
    var primaryDate;
    var pdKey;
    debug('dateStandardizer [%s] > Setting primaryDate',e.sys.id);
    if ( published.isValid() ){
      primaryDate = published.toDate();
      debug('dateStandardizer [%s] > published is valid',e.sys.id);
      pdKey = "published";
    } else {
      primaryDate = created.toDate()
      debug('dateStandardizer [%s] > published is NOT valid',e.sys.id);
      pdKey = "created";
    }
    debug('dateStandardizer [%s] > returning "%s" as primaryDate with value: %s ',e.sys.id,pdKey,primaryDate);
    return primaryDate;
  }
